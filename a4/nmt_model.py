...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.h_projection = nn.Linear(2*hidden_size, out_features=hidden_size, bias=False)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        dec_state = dec_init_state   #init_hidden = (b, h), init_cell = (b, h)

        # Initialize previous combined output vector o_{t-1} as zero
        batch_size = enc_hiddens.size(0)  # enc_hiddens = (b, src_len, h*2) => batch_size = len[0]
        o_prev = torch.zeros(batch_size, self.hidden_size, device=self.device) #matrix shape: [b,h] = 0
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ### END YOUR CODE
        dec_hidden, dec_cell = self.encoder(Ybar_t, dec_state)          #(b,h)
        dec_state = (dec_hidden, dec_cell)
        e_t = torch.bmm(torch.unsqueeze(dec_state,1), enc_hiddens_proj.permute(0,2,1))  #(b,1,h) * (b,h,len) = (b,1,len)
        e_t = torch.squeeze(e_t,1)                   #(b, len)
        ...
        a_t = torch.bmm(torch.unsqueeze(alpha_t,1), enc_hiddens)  #(b,1,2h)
        a_t = torch.squeeze(a_t, 1)  #(b,2h)
        U_t = torch.cat((a_t, dec_hidden), 1)  #(b,3h)
        V_t = self.combined_output_projection(U_t)    #(b,3h)*(3h,h) = (b,h)
        O_t = self.dropout(torch.tanh(V_t))        #(b,h)
        ### END YOUR CODE

        combined_output = O_t
        return dec_state, combined_output, e_t   #((b,h), (b,h)), (b,h), (b,len)
